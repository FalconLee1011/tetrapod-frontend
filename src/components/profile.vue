<template>
  <v-card class="mx-auto" style="margin: 1rem 0" max-width="900px">
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn color="error" style="color: black" light @click="cancel">
        取消
      </v-btn>
      <v-btn color="primary" style="color: black" light @click="change">
        {{ btn1 }}
      </v-btn>
    </v-card-actions>
  </v-card>
...
export default {
  ...
  methods: {
    ...
    fetchData: async function () {
      const res = await this.$axios.post(`${API_PREFIX}/accounts/get-user`, {
        account: this.$store.getters.account,
      });
      const account = res.data.account;
      console.log(account);
      this.avatar_file = account.account_avator;
      this.avatar = await this.generateBlob(account.account_avator);
    ...
    upload: async function () {
      ...
      for (let idx = 0; idx < this.files.length; idx++)
        data.append("account_avator[]", this.files[idx]);
